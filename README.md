...
## Our Entry — Reproducing the Forecast

This fork is a contest entry. The model and methodology are documented in
[METHODOLOGY_3.md](METHODOLOGY_3.md); the competition report is in
[submission/NHS_TabPFN_report.md](submission/NHS_TabPFN_report.md). To reproduce the forecast from a
fresh clone, run everything **from the repo root**:

```bash
# 1. Fetch the dataset (it is stored via Git LFS) and unzip it into data/
git lfs pull
unzip data/turingAI_forecasting_challenge_dataset.csv.zip -d data/
#   -> produces data/turingAI_forecasting_challenge_dataset.csv (~1.3 GB, git-ignored)

# 2. Build the processed feature table (R). Regenerates data/processed_tabpfn.csv
Rscript NHS_TabPFN_data_prep.R

# 3. Generate the 10-day forecasts (Python) -> submission/pred_matrix.csv
python NHS_TabPFN_pipeline.py --mode assess
```

Notes:
- The raw CSV and the processed table are **not** committed (they are large/derived); they are
  regenerated by steps 1–2. Only the LFS zip is stored in the repo.
- Python deps: `numpy`, `pandas`, and `tabpfn` (local CPU engine, no API key needed). Add `--api` to
  step 3 to use the cloud `tabpfn_client` instead.
- On assessment day (data released 6 Jun 2026), just drop the updated dataset in place of the zip's
  contents and re-run — no code edits are needed (the date filter already spans the assessment
  window and `-9999` dummies are handled automatically).
